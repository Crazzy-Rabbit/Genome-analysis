...
- [基因差异表达分析](#差异表达分析)
  - [1.文件下载](#文件下载)
  - [2.循环把下载的所有sra文件都变为fastq-双端测序数据使用--split-3](#循环把下载的所有sra文件都变为`fastq`-双端测序数据使用`--split-3`)
  - [3.`fastp`质量控制](#`fastp`质量控制)
  - [4.参考基因组下载及建索引-如已有，忽略此步](#参考基因组下载及建索引-如已有，忽略此步)
  - [5.reads比对到参考基因组-----hisat2](#reads比对到参考基因组-----hisat2)
  - [6.转为bam文件并排序](#转为bam文件并排序)
  - [7.为`sort.bam`文件建索引](#为`sort.bam`文件建索引)
  - [8.`faturecount`计数定量](#`faturecount`计数定量)
  - [9.对featureCounts的结果进行整合，html文件可视化](#对featureCounts的结果进行整合，html文件可视化)
  - [10.提取定量信息](#提取定量信息)
  - [11.`DESeq2`进行差异分析](#将矩阵导入R中，采用`DESeq2`进行差异分析)
    - [`Clusterprofiler`进行GO及KEGG富集分析](#`Clusterprofiler`进行GO及KEGG富集分析)
    - [好看的富集气泡图的绘制](#好看的富集气泡图的绘制)
  - [12.计算FPKM与TPM,这个值不用于deseq2进行差异表达基因的分析](#计算FPKM与TPM,这个值不用于deseq2进行差异表达基因的分析)
...
```
## 循环把下载的所有sra文件都变为`fastq`-双端测序数据使用`--split-3`
```
...
```
## 对featureCounts的结果进行整合，html文件可视化
```
...
query(hub, "Ovis aries")                                                                         # 查找目标物种的数据库，找.sqlite文件对应的AH号
...
p1 = p + geom_point(aes(size=Count,color=-0.5*log(pvalue),shape=ONTOLOGY,))+
  scale_color_gradient(low = "SpringGreen", high = "DeepPink")                                      # 三个数据分别是以count为点的大小，以颜色的由嫩绿到深粉红代表P值的对数值，以点的性状代表GO类型（MF,CC,BP） 
p1
...
p2
```
## 12 计算FPKM与TPM,这个值不用于deseq2进行差异表达基因的分析
```
# FPKM的计算
# 1 数据载入
rm(list=ls()) 
